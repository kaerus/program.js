...
function TelnetStream(socket) {
    ...
    /* negotiate options */
    this.say("IAC DO TTY");
    this.say("IAC DO NAWS");
};

tools.extend(true,TelnetStream.prototype,stream);
...
TelnetStream.prototype.process = function(data){
    ...
    for(var i = 0, l = data.length; i < l; i++){
        ...
        if(iac || (data[i] === $.IAC && data[i+1] !== $.IAC)){
            ...
            if(Program.debug) debug(0,data[i]+' ');
...
TelnetStream.prototype.say = function(){
    ...
    if(!arguments.length) return CMD;
...
TelnetStream.prototype.setRawMode = function(enable){
    if(enable){
        this.say("IAC WILL ECHO");
        this.say("IAC WILL SUPPRESS_GA");
        this.say("IAC WONT LINEMODE");
    } 
};
