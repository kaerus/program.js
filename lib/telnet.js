...
TelnetStream.prototype._transform = function(data,encoding,callback){
	...
	if(!state && chr !== CMD.IAC){
	    output+=String.fromCharCode(chr);
	}
	else if(state || chr === CMD.IAC){
	    if(!state) {
		state = chr;
	    }
		...
		case CMD.SB:
			...
			self._telnet.suboption.value.push(chr);
		    } else self._telnet.suboption = {type:chr, value:[]};
		    break;
		case CMD.SE:
