...
// mixin event emitter capabilities
Program.__proto__ =  EventEmitter.prototype;
...
Object.defineProperty(Program,'application',{
	...
	if(application.output) Program.scope.output = application.output;
	if(application.input) Program.scope.input = application.input;
	
        return application;
    }
});

var scope = new Scope(util._extend({
    prompt: "$> ",
    debug: Program.debug
},application) );
...
Program.daemonize = require('./daemonize');
...
function evaluateOption(option,value){
    var v, o = Object.keys(option).filter(function(f){return f ? f : void(0);})[0];
    ...
    if(typeof option[o] !== 'object') {
        ...
        var type = Object.keys(option[o]).filter(function(f){return f ? f : void(0);})[0];
        var v1 = o[type], v2;
