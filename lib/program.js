...
Program.run = function(lines){
   var error, 
    ...
    if(error) {
        console.error("Break on line %s:", i, error.stack);
    }
};
...
function evaluateOption(option,value){
    ...
    if(!value) {
        if(!option[""]) throw new Error("Missing parameter: "+o+' '+f+'="'+option[o][f]+'"');
