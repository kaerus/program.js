...
Object.defineProperty(Program,'application',{
    ...
    set: function(n){
        return Program.scope.name = n;
    }
});

Program.log = new Logger(Program.application);

Program.scope.log = Program.log;

Program.banner = function() {
    var s = this.package.name + " version " + this.package.version + "\n";
    
    s+= "Description: " + this.package.description + "\n";
    s+= "Created by: " + this.package.author.name + " <" + this.package.author.email + ">\n";
...
function parse(path,context,scope) {
    ...
    if(typeof command === 'function') {
        ...
        scope.printf(Program.help(path));
    }  
}
