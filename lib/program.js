...
function getParams(node,args) {
    ...
    options.forEach(function(option){
        ...
        try {
            ...
            throw new Error('Option ' + o + '(' + Object.keys(option[o])[0] + '): ' + error.message);
        }    
...
function evaluateOption(option,value){
    var o = Object.keys(option).filter(function(f){if(f)return f})[0],
        f = Object.keys(option[o]).filter(function(f){if(f)return f})[0];

    if(!value) {
        if(!option.hasOwnProperty("")) throw new Error("Missing parameter: "+o+' '+f+'="'+option[o][f]+'"');
...
function getHelp(context) {
    ...
    if(!context) {
        ...
        var c = context = findContext(context);
        if(context) {
            context = getSubNodes(context);
            if(!context.length) context = c;
        }    
    }
